...
เวลาในเอกสารเป็น **เวลาไทย (UTC+7) วันที่ 25 กันยายน 2026** ตามกำหนดการที่ได้รับ. C จดการเปลี่ยนแปลงจากผู้จัดทันที. ใช้ [brief](brief.md) และ [decisions](decisions.md) เป็นแหล่งข้อเท็จจริงระหว่างแข่ง; `Pasted text.txt` เป็นบริบทเก่า ไม่ใช่กติกาที่ได้รับการยืนยัน
...
| 10–20 | อ่าน 3 API และถามกฎเงิน/limit | ร่าง 3 หน้าหลัก | จดเกณฑ์, deadline, AAD/DB/API standard | `/cds-discovery`, `cds-contract` | คำถามที่กระทบ correctness ถูกยื่นใน Q&A |
...
|---|---|---|---|---|---|
| 08:30–09:45 | ฟังโจทย์/จดกฎ business | จด scope หน้า UI | จดกติกา, สิ่งที่ส่ง, cutoff | `/cds-discovery`, `cds-contract` | ปรับ `brief.md`, แยก confirmed/pending |
| 09:45–10:00 | ถาม DB2/limit/state | ถาม branch/center UI | ถาม AAD, API standard, AI/skill policy, deadline | `cds-contract` | คำตอบลง `decisions.md` |
...
| 13:00–14:30 | receipt transaction, idempotency, audit, scope | receipt detail/review/confirm/error | ตรวจ AAD API authorization และ state fixtures | `/cds-receipt-hardening`, `/cds-frontend-slice`, `/cds-integration`; `cds-contract`, `cds-ui-review` | success และ duplicate/cross-branch rejection แสดงได้ |
...
| 16:00–17:00 | แก้ defect ที่ขวางเดโม | browser rehearsal, narrow layout | clean start, diagram/ADR, demo script | `/cds-rehearse`; `cds-verification`, `cds-architecture-defense`, `cds-ui-review` | เพื่อนร่วมทีมรันตาม README ได้; demo สำเร็จ 1 รอบ |
...
5. ลด dashboard/กราฟ/อนิเมชัน, Center functions ที่ไม่อยู่ใน 3 API, Redis/queue/replica, สไลด์ที่ไม่ช่วยเดโม
